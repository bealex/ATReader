...
`BookLayout.shortfall(of:)` says how short a page stands. Each sheet is cut again only once, so a
page the rules do leave short stays as it came.

A debug report writes `layout.txt` beside the page: the window and the sheet the
